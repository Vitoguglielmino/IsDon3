...
//  Il mio prototipo
//
//  Created by Vito Guglielmino on 08/01/22.
//
...
class ThirdViewController: UIViewController, UIApplicationDelegate {
    ...
    @IBOutlet weak var Menu_Prova: UIMenu!
    @IBOutlet weak var addButton: UIButton!
    @IBOutlet weak var pageStudiedText: UITextField!
...
//        self.viewDidLoad()
        ...
        getSubject()
        removeAction()
        Prova.menu = ._nilValue()
        createItems()
        
        if actions.isEmpty == true {
            Prova.isEnabled = false
        }else {
        
  //      let menù = UIMenu(title: "", children: actions.self)
            Prova.isEnabled = true
        Prova.menu = UIMenu(title: "", children: actions.self)
        }
